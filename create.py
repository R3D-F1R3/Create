...
------------------------------------------
 Author : RED-FIRE
 Github : R3d-F1r3 (Follow Must)
 Tools  : Auto Facebook Register
 Contact: +8801406103107
 Version: 1.6
\033[1;37m------------------------------------------""") 
try:
    clear()
    ...
    if "Version-1.5" in response:
        ...
        if kex in response:
            ...
            linex()
            print(" You cannot run this tool without 𝙍𝙀𝘿🝮︎︎︎︎︎︎︎𝙊𝙉𖤍𝙁𝙄𝙍𝙀♲︎︎︎ permission  \n Note: you have any problem in this tool,,  fast inbox me")
            linex()
            print(" Your key: "+fkeyx);subprocess.check_output(["am", "start", "https://api.whatsapp.com/send?phone=+8801406103107&text="+(" Hi Sagor sir i want Buy Auto Create Ids Tool Please Approve My Token\n Token:- "+ fkeyx)]);time.sleep(2)
...
def Create():
    ...
    def strt():
       try:
           ...
           import sys
           loop+=1
           sys.stdout.write('\r\r\033[1;37m [𝙍𝙀𝘿-𝙁𝙄𝙍𝙀-CREATE] \033[1;32mOK:%s \033[1;37m'%(ok));sys.stdout.flush()
           requests=r.Session()
           from fake_email import Email
           mmail=Email().Mail()
           em=mmail['mail']
           ...
           if "9":
              ...
              if requests=="chk":
                ...
                pass
                 ...
                 print("\r\r\033[1;32m [𝙍𝙀𝘿-𝙁𝙄𝙍𝙀-OK] "+uid+'|'+pas+'|'+coki)
                 ok+=1
           ...
           pass
    file="/sdcard/𝙍𝙀𝘿-𝙁𝙄𝙍𝙀/CREATE-OK.txt"
    u=5000
    clear()
    print(' Use Airplane Or Use IP Changer if ids coming cp ')
    print(' ids will be save in /sdcard/𝙍𝙀𝘿-𝙁𝙄𝙍𝙀/CREATE-OK.txt')
    linex()
